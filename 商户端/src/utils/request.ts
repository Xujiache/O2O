 ...
 *
 * P5 经验教训：axios 在 mp-weixin 不稳定，必须用 uni.request；
 *               商户端复用用户端的同款封装，并附加：
 *   - X-Client-Type: merchant（必带）
 *   - X-Mchnt-Shop-Id: <shopId>（多店切换；当前店铺 store 同步时自动注入）
 *   - X-Idem-Key: <hex>（写操作幂等）
...
/** 当前店铺 ID 提供器（避免 store 循环依赖；store 初始化时注入） */
let currentShopIdProvider: (() => string | null) | null = null

/**
 * 注入当前店铺 ID 提供器（由 shop store 初始化时调用）
 * 功能：让 request 在不直接 import shop store 的前提下注入 X-Mchnt-Shop-Id 头
 * @param provider 返回当前店铺 ID 的函数；返回 null 则不注入
 */
export function setShopIdProvider(provider: () => string | null): void {
  currentShopIdProvider = provider
}
...
/**
 * 错误码 toast 文案表（节选 P4 §10.4 错误码字典 + 商户端常见）
 */
...
function rawRequest<T = unknown>(opt: RequestOptions): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    /* 自动注入当前店铺 ID（多店切换上下文） */
    const shopId = currentShopIdProvider?.()
    if (shopId) {
      headers['X-Mchnt-Shop-Id'] = shopId
    }
...
/**
 * PATCH 快捷方法（uni.request 不支持 PATCH，降级为 POST + X-Method-Override 头）
 * 后端 NestJS 通常通过 method-override 中间件识别该头部
 */
...
/* ===== 兼容 P1 骨架的命名（rootRequest / 旧 fetchHealth 调用） ===== */
