 ...
 * @stage P9/W5.E.4 (Sprint 5) — 商户端推送 token 注册
 * @desc 商户端简单注册器（mock 优先，本期不接极光 SDK）：
...
const DEVICE_ID_KEY = 'O2O_MERCHANT_PUSH_DEVICE_ID'
...
function getRegistrationId(): string {
  ...
  return `mock-rid-merchant-${getOrCreateDeviceId().slice(0, 12)}`
}
