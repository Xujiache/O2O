...
/**
 * 读取 Vite 注入的运行时环境变量
 * 功能：屏蔽 uni-app / Vite / 原生小程序环境变量差异
 * 参数：key 环境变量键名
 ...
 * 功能：所有业务 API 请求走同一实例，便于统一处理 token、错误、loading、埋点
 * 参数：无
 * 返回值：AxiosInstance 可直接调用 .get/.post 的实例
 * 用途：P5+ 业务 API 封装统一依赖此实例
 */
